...

{
    ...
    if (Tree->Root.LeftChild->Parent != &(Tree->Root)) {
        printf("Error: Tree 0x%x root 0x%x (NullNode 0x%x) "
...

{
    ...
    if (Node->Red != FALSE) {
        if ((Node->LeftChild->Red != FALSE) ||
            ...
            printf("Error: Red-Black Tree 0x%x has a red node 0x%x "
    ...
    if (Node->LeftChild != &(Tree->NullNode)) {
        ...
        if ((CompareResult != ComparisonResultSame) &&
            ...
            printf("Error: Red-Black Tree 0x%x has a node 0x%x whose "
        ...
        if (Node->LeftChild->Parent != Node) {
            printf("Error: Node 0x%x LeftChild 0x%x Parent is 0x%x "
    ...
    if (LeftBlackCount != RightBlackCount) {
        printf("Error: Red-Black Tree 0x%x has a node 0x%x with a left "
